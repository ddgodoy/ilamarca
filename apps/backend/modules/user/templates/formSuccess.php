		...
		<form enctype="multipart/form-data" method="post" action="<?php echo url_for($str_module.'/'.$str_action).$request_id ?>">
			...
			<fieldset>
				<table width="100%" cellspacing="4" cellpadding="0" border="0">
					<tr>
						<?php if ($sf_user->getAttribute('user_role') == 'super_admin'): ?>
							<td><label><?php echo __('Company') ?> *</label></td>
							<td>
								<select name="company" class="form_input" style="width:330px;">
									<?php echo Common::fillSimpleSelect(CompanyTable::getInstance()->getAllForSelect(true), $company) ?>
								</select>
							</td>
						<?php else: ?>
							<td colspan="2"><input type="hidden" name="company" value="<?php echo $sf_user->getAttribute('user_company') ?>" /></td>
						<?php endif; ?>
					</tr>
					<tr>
						<td><label><?php echo __('User role') ?> *</label></td>
						<td>
							<select name="user_role" class="form_input" style="width:330px;">
								<?php echo Common::fillSimpleSelect(UserRoleTable::getInstance()->getAllForSelect(true), $user_role) ?>
							</select>
						</td>
					</tr>
					<tr>
